...
def save_predictions_weighted_avg(model_a, model_b, images_dir, masks_dir, output_dir, transform_patch, alpha):
    ...
    with torch.no_grad():
        for idx, (img_name, mask_name) in enumerate(zip(image_names, mask_names)):
            ...
            for patch, (y, x) in patches:
                ...
                prob_a = torch.sigmoid(model_a(input_tensor))[0, 0].cpu().numpy()
                prob_b = torch.sigmoid(model_b(input_tensor))[0, 0].cpu().numpy()

                combined_prob = alpha * prob_a + (1 - alpha) * prob_b
                binary_mask = (combined_prob > THRESHOLD).astype(np.uint8) * 255

                final_patches[(y, x)] = binary_mask
            ...
            combined_overlay = cv2.addWeighted(image, 0.5, overlay, 0.5, 0)
            ...
            cv2.imwrite(os.path.join(output_dir, f"weightedavg_{idx}_{img_name}"), combined_image)

    pd.DataFrame(results).to_csv(os.path.join(output_dir, "weighted_avg_iou_metrics.csv"), index=False)
...
if __name__ == "__main__":
    ...
    output_dir = "C:/Users/mirom/Desktop/exp weighted/0.7"
    ...
    save_predictions_weighted_avg(model_large, model_small, images_dir, masks_dir, output_dir, transform_patch,
                                   alpha=0.7)
