...
from my_utils.metrics import compute_iou, compute_f1
...
from my_utils.metrics import compute_iou, compute_f1  # импорт из твоего модуля

def save_predictions_dual_patch_area_threshold(model_large, model_small, images_dir, masks_dir, output_dir, transform_patch, area_threshold=200, alpha=0.5):
    ...
    with torch.no_grad():
        for idx, (img_name, mask_name) in enumerate(zip(image_names, mask_names)):
            ...
            for patch, (y, x) in patches:
                ...
                out_large = model_large(input_tensor)[0, 0].cpu().numpy()
                out_small = model_small(input_tensor)[0, 0].cpu().numpy()

                bin_large = (torch.sigmoid(torch.tensor(out_large)) > 0.55).numpy().astype(np.uint8)
                bin_small = (torch.sigmoid(torch.tensor(out_small)) > 0.55).numpy().astype(np.uint8)

                labeled_array, num_features = label(np.maximum(bin_large, bin_small))
                final_combined = np.zeros_like(bin_large)

                for label_idx in range(1, num_features + 1):
                    region = (labeled_array == label_idx)
                    area = region.sum()
                    if area > area_threshold:
                        final_combined[region] = bin_large[region]
                    else:
                        final_combined[region] = bin_small[region]

                final_patch = (final_combined * 255).astype(np.uint8)
                final_patches[(y, x)] = final_patch

            final_prediction = merge_patches(final_patches, image.shape[:2])

            pred_tensor = torch.tensor(final_prediction > 0).unsqueeze(0).unsqueeze(0).float()
            mask_tensor = torch.tensor(mask > 0).unsqueeze(0).unsqueeze(0).float()

            iou = compute_iou(pred_tensor, mask_tensor, threshold=0.5)
            f1 = compute_f1(pred_tensor, mask_tensor, threshold=0.5)

            results.append({"image_name": img_name, "IoU": iou, "F1": f1})
            ...
            cv2.imwrite(os.path.join(output_dir, f"dualpatch_area_{idx}_{img_name}"), combined_image)

    pd.DataFrame(results).to_csv(os.path.join(output_dir, "dualpatch_area_iou_metrics.csv"), index=False)
...
if __name__ == "__main__":
    ...
    output_dir = "C:/Users/mirom/Desktop/exp ensemble thold/25k"
    ...
    save_predictions_dual_patch_area_threshold(model_large, model_small, images_dir, masks_dir, output_dir,
                                               transform_patch, area_threshold=25000, alpha=0.5)
