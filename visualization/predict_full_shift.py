...
def split_into_patches(image: np.ndarray, patch_size: int = PATCH_SIZE) -> list:
    patches = []
    h, w = image.shape[:2]
...
def merge_patches(patches: dict, image_shape: tuple, patch_size: int = PATCH_SIZE) -> np.ndarray:
    ...
    with torch.no_grad():
        for idx, (img_name, mask_name) in enumerate(zip(image_names, mask_names)):
            ...
            for patch, (y, x) in patches:
                ...
                pred_1 = torch.sigmoid(model(input_tensor))[0, 0].cpu().numpy()

                shifted_patch = apply_shift(patch, SHIFT_X, SHIFT_Y)
                shifted_aug = transform(image=shifted_patch)
                shifted_tensor = shifted_aug["image"].unsqueeze(0).to(my_utils.config.DEVICE)
                pred_2 = torch.sigmoid(model(shifted_tensor))[0, 0].cpu().numpy()
                pred_2_unshifted = apply_shift((pred_2 * 255).astype(np.uint8), -SHIFT_X, -SHIFT_Y).astype(np.float32) / 255.0

                combined_pred = ((pred_1 > 0.65) | (pred_2_unshifted > 0.55)).astype(np.uint8) * 255
                pred_patches[(y, x)] = combined_pred
...
if __name__ == "__main__":
    images_dir = "D:/bp_dataset/base_subsets/train/images"
    masks_dir = "D:/bp_dataset/base_subsets/train/masks"
    output_dir = "C:/Users/mirom/Desktop/full_preds_large_shift"
    ...
    model_path = "../train/test_valid_small_300.pth"
