...
def save_predictions_on_patches_ensemble_or(model_large, model_small, images_dir, masks_dir, output_dir, transform_patch, transform_full, alpha=0.5):
    ...
    with torch.no_grad():
        for idx, (img_name, mask_name) in enumerate(zip(image_names, mask_names)):
            ...
            # --- Large model on full image ---
            full_aug = transform_full(image=image)
            full_input_tensor = full_aug["image"].unsqueeze(0).to(my_utils.config.DEVICE)
            out_large = torch.sigmoid(model_large(full_input_tensor))[0, 0].cpu().numpy()

            # --- Small model on patches ---
            ...
            for patch, (y, x) in patches:
                ...
                pred_patches[(y, x)] = (out_small > 0.55).astype(np.uint8) * 255

            small_prediction = merge_patches(pred_patches, image.shape[:2])
            large_prediction = (out_large > 0.55).astype(np.uint8) * 255

            final_prediction = np.maximum(small_prediction, large_prediction)

            iou = compute_iou(final_prediction > 0, mask > 0)
            ious.append({"image_name": img_name, "IoU": iou})
            ...
            cv2.imwrite(os.path.join(output_dir, f"ensemble_{idx}_{img_name}"), combined_image)

    pd.DataFrame(ious).to_csv(os.path.join(output_dir, "ensemble_iou_metrics.csv"), index=False)


if __name__ == "__main__":
    images_dir = my_utils.config.TEST_IMAGES_DIR
    masks_dir = my_utils.config.TEST_MASKS_DIR
    output_dir = "C:/Users/mirom/Desktop/ensemble_or_preds"
    ...
    checkpoint_large = torch.load("../train/efnet7small.pth", map_location=my_utils.config.DEVICE)
    ...
    save_predictions_on_patches_ensemble_or(model_large, model_small, images_dir, masks_dir, output_dir, transform_patch, transform_full)
